        ...
        try:
            ...
            if excel_refs['total_row_index'] is not None:
                ...
                self._create_modern_kpi_card_with_formula(
                    ...
                    f'=IFERROR(ROUND(((총액!{excel_refs["center_col"]}{excel_refs["total_row_index"]}+총액!{excel_refs["research_col"]}{excel_refs["total_row_index"]})-(총액!E2+총액!F2+총액!E3+총액!F3))/(총액!{excel_refs["budget_col"]}{excel_refs["total_row_index"]}-총액!D2-총액!D3)*100,1)&"%","0%")',  # 인건비를 제외한 집행률 계산 (인건비 예산 및 집행액 제외)
...
class InteractivePivotGenerator:
    ...
    def add_interactive_features(self, file_path: str) -> bool:
        ...
        try:
            ...
            with xw.App(visible=True) as app:
                ...
                # 피벗용 별도 시트(`예산분석`)는 더 이상 필요합니다. 대신
                # 피벗은 세로형 데이터 시트(`예산분석데이터`)의 E5에 생성합니다.
                pivot_table = self._create_pivot_table(wb, '총액', None, pivot_data_sheet='예산분석데이터', dest_cell='E5')
                if not pivot_table:
                    logging.error("피벗 테이블 생성 실패")
                    return False

                # 피벗 차트 추가 (ws_pivot is None because pivot is on pivot_data sheet)
                #self._add_pivot_chart(None, pivot_table)

                # 슬라이서 추가
                #self._add_slicers(wb, pivot_table, None)
    ...
    def _create_pivot_table(self, wb, source_sheet_name: str, ws_pivot, pivot_data_sheet: str = None, dest_cell: str = 'A3') -> object:
        '''피벗 테이블 생성'''
        try:
            import xlwings as xw
            ...
            # 먼저 세로형 데이터 시트 생성
            # If caller provided a pivot_data_sheet name, ensure it exists by creating/updating it.
            if pivot_data_sheet:
                pivot_data_sheet_name = self._create_pivot_data_sheet(wb, source_sheet_name)
                # If the created sheet name differs from requested name, try to rename it
                try:
                    if pivot_data_sheet_name and pivot_data_sheet_name != pivot_data_sheet:
                        ws = wb.sheets[pivot_data_sheet_name]
                        ws.name = pivot_data_sheet
                        pivot_data_sheet_name = pivot_data_sheet
                except Exception:
            ...
            # Determine target worksheet and destination cell for the pivot
            if pivot_data_sheet_name:
                try:
                    ws_target = wb.sheets[pivot_data_sheet_name]
                except Exception:
                    ws_target = ws_pivot
            else:
                ws_target = ws_pivot

            dest = dest_cell or 'A3'

            # 2. 피벗 테이블 생성
            logging.info(f"피벗 테이블 생성 중... 대상: {ws_target.name} {dest}")
            pivot_table = pivot_cache.CreatePivotTable(
                TableDestination=ws_target.range(dest).api,
    ...
    def _add_pivot_chart(self, ws_pivot, pivot_table):
        ...
        try:
            ...
            # 피벗 차트 생성 (세로 막대형)
            # ws_pivot may be None if pivot is created on the pivot_data sheet;
            # in that case derive the worksheet from the pivot_table object.
            if ws_pivot is None:
                try:
                    ws_for_chart = pivot_table.TableRange2.Parent
                except Exception:
                    ws_for_chart = pivot_table.Parent
            else:
                ws_for_chart = ws_pivot.api

            chart_shape = ws_for_chart.Shapes.AddChart2(
                227,  # 차트 스타일
                xw.constants.ChartType.xlColumnClustered
            )
            chart = chart_shape.Chart
            # Use the pivot table range as source
            try:
                src_range = pivot_table.TableRange2
                chart.SetSourceData(Source=src_range)
            except Exception:
                # fallback: use A3 expansion on the resolved worksheet
                chart.SetSourceData(Source=ws_for_chart.Range('A3').Expand().api)
            chart.HasTitle = True
    ...
    def _add_slicers(self, wb, pivot_table, ws_pivot):
        ...
        try:
            ...
            try:
                ...
                # Resolve slicer destination worksheet
                if ws_pivot is None:
                    try:
                        dest_ws_api = pivot_table.TableRange2.Parent
                    except Exception:
                        dest_ws_api = pivot_table.Parent
                else:
                    dest_ws_api = ws_pivot.api

                slicer_cache_budget.Slicers.Add(
                    SlicerDestination=dest_ws_api,
            ...
            try:
                ...
                # Resolve slicer destination worksheet for characteristics slicer
                if ws_pivot is None:
                    try:
                        dest_ws_api = pivot_table.TableRange2.Parent
                    except Exception:
    ...
    def _add_analysis_pivot_and_slicers_to_dashboard(self, wb, ws_dashboard):
        ...
        try:
            ...
            # 우선적으로 피벗이 실제로 생성된 데이터 시트(`예산분석데이터`)에서 피벗을 찾습니다.
            pivot_com = None
            source_range = None
            pivot_data_sheet_name = '예산분석데이터'

            if pivot_data_sheet_name in [s.name for s in wb.sheets]:
                ws_pivot_data = wb.sheets[pivot_data_sheet_name]
                # 시트에 실제로 피벗 테이블 COM 객체가 있는지 시도
                try:
                    pivot_com = ws_pivot_data.api.PivotTables(1)
                    # Pivot COM provides TableRange2
                    try:
                        source_range = pivot_com.TableRange2
                    except Exception:
                        source_range = ws_pivot_data.range('E5').expand().api
                except Exception:
                    # 피벗 COM이 없을 경우 E5에서 확장 범위를 시도
                    try:
                        source_range = ws_pivot_data.range('E5').expand()
                    except Exception:
                        source_range = None

            # 이전 방식: 예산분석 시트에 직접 피벗이 있을 때의 처리 (하위 호환)
            ...
            chart = chart_shape.Chart
            # source_range may be a COM TableRange2 or an xlwings Range
            try:
                if hasattr(source_range, 'Address'):
                    # COM TableRange2 or api range
                    chart.SetSourceData(Source=source_range)
                else:
                    chart.SetSourceData(Source=source_range.api)
            except Exception:
                # fallback: try using pivot sheet E5 expand
                try:
                    chart.SetSourceData(Source=wb.sheets[pivot_data_sheet_name].range('E5').expand().api)
                except Exception as e:
                    logging.error(f'예산분석 차트 소스 설정 실패: {e}')
                    return
            chart.HasTitle = True
            ...
            try:
                ...
                # Prefer using the actual pivot COM object if available
                if pivot_com is not None:
                    try:
                        slicer_cache_metric = wb.api.SlicerCaches.Add2(pivot_com, '특성')
                    except Exception as add_err:
                        logging.info(f"측정항목 슬라이서 캐시 추가 실패(이미 존재할 수 있음): {add_err}")
                        slicer_cache_metric = None

                # If not created, try to find existing slicer cache by name
                if slicer_cache_metric is None:
                    try:
            ...
            try:
                ...
                if pivot_com is not None:
                    try:
                        slicer_cache_budget = wb.api.SlicerCaches.Add2(pivot_com, '예산과목')
                    except Exception as add_err:
                        logging.info(f"예산과목 슬라이서 캐시 추가 실패(이미 존재할 수 있음): {add_err}")
                        slicer_cache_budget = None

                if slicer_cache_budget is None:
                    try:
                        sc_count = int(wb.api.SlicerCaches.Count)
                        for idx in range(1, sc_count + 1):
                            sc = wb.api.SlicerCaches.Item(idx)
